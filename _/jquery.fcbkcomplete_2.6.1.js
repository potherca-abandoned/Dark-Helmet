		        ...
		        function preSet()
		        {
		            element.children("option").each(
		                function(i,option)
		                {
		                    ...
		                    {
/**/                            addItem({"title":option.text(), "value":option.val(), "addClass":option.attr('class')}, true);
				...
				{
/**/				addItem({'title':data.title, 'value':data.value, 'addClass':data.addClass});
				});

//            function addItem (title, value, preadded)
/**/            function addItem (object, preadded) // title, value
		        {
//console.trace('addItem()');
/**/                if(!object.title || !object.value){
/**/                    //we must at least have a title and value!
/**/                    throw('Object missing title or value variable');
/**/                }else{

                        var li = document.createElement("li");
    /**/                var txt = document.createTextNode(object.title);
                        var aclose = document.createElement("a");

    /**/                $(li).attr({"rel": object.value});
//console.info(object);
    /**/                if(object.addClass){
    /**/                    $(li).addClass('bit-box ' + object.addClass);
    /**/                }else{
    /**/                    $(li).addClass('bit-box');
    /**/                }
                        $(li).prepend(txt);
                        ...
                        if (!preadded)
                        {
                            ...
                            addInput(1);
    /**/                    if (element.children("option[value=" + object.value + "]").length)
                            {
    /**/						_item = element.children("option[value=" + object.value + "]");
    ...
    /**/                        _item.attr("value", object.value).get(0).setAttribute("selected", "selected");
    /**/						_item.attr("value", object.value).addClass("selected");
    /**/                        _item.text(object.title);
...
/**/                }//if
				...
				function removeItem(item)
				{
					...
					{
					    _item = element.children("option[value=" + item.attr("rel") + "]");
		        ...
		        function addInput(focusme)
		        {
	                ...
	                input.blur(
                        //@FIXME: Check if mouse pointer isn't, in fact, over the scrollbar!
	                    function() {
...
/**/                                    addClass: val.addClass
					...
					while (match != null && maximum > 0)
					{
						...
						{
/**/                        content += '<li rel="' + object.value + '" class="' + object.addClass + '"' + '">'
		        ...
		        function bindEvents()
		        {
	                ...
	                feed.children("li").mousedown(
	                    function()
	                    {
	                        var option = $(this);
//console.warn(option);
/**/                        addItem({'title':option.text(), 'value':option.attr("rel"), "addClass":option.attr('class')});
	                        feed.hide();
							browser_msie?browser_msie_frame.hide():'';
	                        complete.hide();
	                    }
